...
const IndexPage = () => {
    ...
    const [gameResult, setGameResult] = useState(''); // Add a state variable to store the result
    ...
    const playGame = async (playerChoice) => {
        ...
        try {
            ...
            if (receipt.events && receipt.events.GameResult) {
                ...
                setGameResult(gameResult); // Update state variable
            } else {
                console.log("Game result event not found in transaction receipt");
                setGameResult("No result found"); // Set a fallback result
            }
        } catch (err) {
            console.error("Error playing the game:", err);
            setGameResult("Error playing the game"); // Set an error message
        }
    };
    ...
    return (
        <div className="flex flex-col items-center justify-center h-screen">
            ...
            <div id="result" className="mt-4">
                {gameResult && <p>Game Result: {gameResult}</p>} {/* Display the game result */}
            </div>
        </div>
    );
};
